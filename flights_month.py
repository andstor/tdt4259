...
for filename in all_files:
    ...
    dfSum = dfBagtags.groupby([dfBagtags['sourceTimestamp'].dt.to_period("M"), 'LegArrayLength', 'Leg0_departureAirportIATA']).size().reset_index().rename(columns={"sourceTimestamp":"month", "Leg0_departureAirportIATA":"startLegIATA"})

    routes = pd.concat((routes,dfSum)).groupby(["month", 'startLegIATA'],as_index=False).sum()


routes['Flights'] = routes['LegArrayLength'] * routes[0]
routes.to_csv('./stats/flights_month.csv', index = False, header=True)
